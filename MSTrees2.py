...
class distance_matrix(object) :
    ...
    def get_distance(func, profiles, handle_missing) :
        ...
        if n_proc > 1 :
            ...
            #subfiles = map(parallel_distance, [[func, params['prof_file'], params['dist_subfile'], handle_missing, idx] for idx in indices])
            pool.close()
            del pool
    ...
    def asymmetric(profiles, handle_missing = 'pair_delete', index_range=None) :
        ...
        if handle_missing not in ('absolute_distance', ) :
            for i2, id in enumerate(np.arange(*index_range)) :
                profile, presence = profiles[id], presences[id]
                diffs = np.sum(((profiles != profile) & presence), axis=1) * float(presence.size)/np.sum(presence)/20.0
                distances[:, i2] = diffs
        else :
            for i2, id in enumerate(np.arange(*index_range)) :
                profile, presence = profiles[id], presences[id]
    ...
    @staticmethod
    def detailed(p1, p2) :
        distance = np.zeros(p1.shape, dtype=int)
        for pId, pp in enumerate(p1) :
            for pJd, (x1, x2) in enumerate(zip(pp, p2)) :
                if x1 == '-' or x2 == '-' :
                    distance[pId, pJd] = 20
                else :
                    xx1 = { xx:1 for xx in x1.split(',') }
                    distance[pId, pJd] = np.sum([1 for xx in x2.split(',') if xx not in xx1])
        return distance
    ...
    def symmetric(profiles, handle_missing = 'pair_delete', index_range=None) :
        ...
        if handle_missing in ('pair_delete',) :
            for i2, id in enumerate(np.arange(*index_range)) :
                ...
                diffs = np.sum(distance_matrix.detailed(profiles[:id], profile) * comparable, axis=1) * float(presence.size) / np.sum(comparable, axis=1)
...
def backend(**args) :
    ...
    names = np.array([re.sub(r'[\(\)\ \,\"\';]', '_', n) for n in names])
    embeded  = {n:[n] for n in names}
